...
- For each supermarket in `scripts/flyers/stores.json`, it downloads **every page** of the flyer from flyers-on-line.com (pages are taken in **document order**, not by the `_NN.jpg` suffix — Superstore's page 1 is `_07.jpg`, `_01.._06` being ad inserts), has Claude (headless CLI) read each page image in a separate call, and appends the extracted deals to the family Firestore doc. A single failed page (download or extraction) is skipped with a log — the store only fails if **all** pages fail. Downloaded images are deleted afterwards. (Was page-1-only until the import whitelist existed to keep volume sane; the per-item+store weekly dedupe also swallows cross-page duplicates.)
