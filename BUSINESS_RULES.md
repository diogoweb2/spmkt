...
| **PhotoQueue entry** | `id, path, storeId, status, ts` (+ extracted fields when `ready`: `itemName, matchedItemId, price, qty, unit, category, frozen/bones/skin/processing` or `groceryType`, `note`; + `error` when `failed`) | 📷 photo mode, see §15. `status`: `pending` \| `ready` \| `failed`. `ts` = when the photo was taken (becomes the record's ts on approve). A ⚡ Photo Live extraction sent to full edit (§15a) is parked here as `ready` with `path: null` (no photo in Storage). |
