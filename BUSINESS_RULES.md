...
- The **➕ FAB** (center of the mobile nav bar; top of the desktop rail) opens an **Android-style speed-dial menu** with two actions: **✏️ Manual entry** (→ AddPrice) and **📷 Photo batch** (→ rear camera, `multiple` capture, each shot queued for §15). Tapping either action first shows the **"Where are you?" store sheet to confirm the store**, then runs the action. The confirmation is **skipped for 1 hour** after each pick (timestamp in localStorage `spmkt.storeConfirmedAt`, per-device, never synced) as long as a current store is set; picking a store also updates `db.currentStoreId`.
